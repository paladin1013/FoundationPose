...
class ClosureFoundationPose(ClosureBase):
    ...
    def get_final_poses(
        ...
        final_Rs:cp.ndarray = cp.matmul(additional_Rs, self.init_Rs)
        final_ts:cp.ndarray = cp.matmul(additional_Rs, self.init_ts[:, None, :]) + additional_ts[:, None, :]
    ...
    def get_nonconformity_residual(
        ...
        nonconformity_scores = self.nonconformity_func(
            *self.get_final_poses(additional_Rs, additional_ts),
            self.pred_Rs, self.pred_ts, self.pred_scores
        ).reshape(additional_Rs.shape[:-2]) # (N, n_walks)
    ...
    def sample_rotation_boundary(self) -> Tuple[cp.ndarray, cp.ndarray]:
        ...
        t_perturbations = self.get_translation_perturbation()  # (n_iterations, n_perturbations, 3)
        ...
        for iter_cnt in range(self.n_iterations):
            ...
            best_t_perturbations = t_perturbation[top_k_indices].reshape(N, self.n_walks, self.n_optimal_perturbations, 3)
